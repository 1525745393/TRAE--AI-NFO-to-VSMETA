...
class Config:
    ...
    video_extensions: List[str] = field(default_factory=lambda: [
        ".mp4", ".mkv", ".avi", ".ts", ".wmv", ".rmvb", ".mov", ".m4v"
    ])
    ...
    # === 输出格式配置 ===
    output_formats: List[str] = field(default_factory=lambda: ['vsmeta'])  # 输出格式列表，支持 vsmeta 和 nfo
...
class NFOParser:
    ...
    def _extract_metadata(self, root: ET.Element, nfo_path: str) -> VideoMetadata:
        ...
        for field, tags in self.FIELD_MAPPINGS.items():
            for tag in tags:
                elem = root.find(f".//{tag}")
                if elem is not None and elem.text:
                    value = elem.text.strip()
                    self._set_field(metadata, field, value)
                    break
...
class VSMETAGenerator:
    ...
    def __init__(self, config: Config):
        ...
        """
        self.config = config
        self._image_cache = ImageCache(max_size=getattr(config, 'image_cache_max_size', 50))  # 使用配置的缓存大小
    ...
    def _matches_filters(self, filename: str) -> bool:
        ...
        if self.config.file_include_patterns:
            matched = any(
                ...
                for pattern in self.config.file_include_patterns
            )
            if not matched:
                return False
        ...
        if self.config.file_exclude_patterns:
            excluded = any(
                fnmatch.fnmatch(filename, pattern)
                for pattern in self.config.file_exclude_patterns
            )
            if excluded:
                return False
    ...
    def load(self):
        ...
        if os.path.exists(self.checkpoint_file):
            try:
                ...
                logger.info(f"已加载断点文件，已完成: {len(self.completed)}, 失败: {len(self.failed)}")
            except Exception as e:
...
class PluginManager:
    ...
    def register(self, plugin: Plugin, global_config: 'Config' = None) -> None:
        ...
        if isinstance(plugin, NFOParserPlugin):
            ...
            self._parser_plugins.sort(key=lambda p: (-getattr(p, 'priority', 50) if not callable(getattr(p, 'priority', None)) else p.priority, p.name))
        if isinstance(plugin, VSMETAGeneratorPlugin):
            self._generator_plugins.append(plugin)
            self._generator_plugins.sort(key=lambda p: (-getattr(p, 'priority', 50) if not callable(getattr(p, 'priority', None)) else p.priority, p.name))
        if isinstance(plugin, MetadataEnhancerPlugin):
            self._enhancer_plugins.append(plugin)
            self._enhancer_plugins.sort(key=lambda p: (-getattr(p, 'priority', 50) if not callable(getattr(p, 'priority', None)) else p.priority, p.name))
        if isinstance(plugin, FileFilterPlugin):
            self._filter_plugins.append(plugin)
            self._filter_plugins.sort(key=lambda p: (-getattr(p, 'priority', 50) if not callable(getattr(p, 'priority', None)) else p.priority, p.name))
        ...
        logger.info(f"插件已注册: {plugin.name} v{plugin.version} (优先级: {pri}) - {plugin.description}")
            ...
            for attr_name in dir(module):
                ...
                if (isinstance(attr, type)
                    ...
                    try:
                        ...
                        logger.info(f"插件热重载成功: {plugin_instance.name} v{plugin_instance.version}")
                    except Exception as e:
...
<body>
    <div class="container">
        ...
        for detail in details:
            result_class = 'success' if detail['result'] == 'success' else 'error' if detail['result'] == 'error' else 'warning'
            rows.append(f"""
                <tr>
...
class NFOToVSMETAConverter:
    ...
    def process_with_checkpoint(self):
        ...
        pending_files = [
            ...
            if not self.checkpoint.is_completed(os.path.join(d, f))
        ]

        if len(pending_files) < len(files):
            print(f"{Fore.GREEN}跳过 {len(files) - len(pending_files)} 个已处理文件{Style.RESET_ALL}")
    ...
    def _process_single_file(self, directory: str, filename: str) -> Dict:
        ...
        if self.config.dry_run:
            if not os.path.exists(nfo_path):
                return {'success': False, 'result': 'nfo_missing', 'error': '[DRY-RUN] NFO 文件不存在'}
        ...
        # === VSMETA 非空验证 ===
        if len(vsmeta_data) == 0:
            return {'success': False, 'result': 'generate_error', 'error': 'VSMETA 生成失败（空数据）'}
    ...
    def _update_stats(self, result: Dict, directory: str, filename: str):
        ...
        with self._stats_lock:
            ...
            if result.get('success'):
                ...
                self.checkpoint.mark_failed(os.path.join(directory, filename), result.get('error', 'Unknown error'))
                self.stats.errors.append(detail)

            # === 插件：文件处理结束生命周期通知 ===
            self.plugin_manager.notify_lifecycle('on_file_end', filepath=os.path.join(directory, filename), result=result)
...
<tr><td>工作线程数</td><td>{self.config.max_workers}</td></tr>
            ...
            writer.writerow(['平均处理时间(秒/文件)', f'{self.stats.duration / max(self.stats.processed_files, 1):.2f}'])
...
def interactive_config_with_validation() -> Config:
    ...
    # 压缩比例
    config.image_compression_ratio = _prompt_float("图片压缩比例 0.1-1.0", config.image_compression_ratio, min_val=0.1, max_val=1.0)
...
def main():
    ...
    parser.add_argument('--plugin-type', choices=['parser', 'generator', 'enhancer', 'filter', 'lifecycle'],
                       default='enhancer', help='插件类型 (默认: enhancer)')
    parser.add_argument('--plugin-output-dir', default='plugins', help='插件输出目录 (默认: plugins)')
    ...
    parser.add_argument('--plugin-priority', type=int, default=50, help='插件优先级 0-100 (默认: 50)')
    ...
    parser.add_argument('--output-formats', help='输出格式列表（逗号分隔），支持 vsmeta 和 nfo，默认为 vsmeta')
    ...
    while True:
        ...
        if idx == len(menu_options) or idx == -1:
            ...
            # 清除断点文件
            confirm = input(f"{Fore.YELLOW}确定要清除断点文件吗？(y/N): {Style.RESET_ALL}").strip().lower()
            ...
            if converter is None or not hasattr(converter, 'report_details'):
                ...
                confirm = input(f"{Fore.YELLOW}确定要重试这些文件吗？(y/N): {Style.RESET_ALL}").strip().lower()
                if confirm not in ('y', 'yes'):
                    print("已取消")
                    continue
            ...
            while True:
                ...
                if plugins:
                    ...
                    for i, p in enumerate(plugins, 1):
                        print(f"  {i}. {p['name']} v{p['version']} [{p['type']}] - {p['description']}")
                else:
                ...
                if choice == '0':
                    ...
                    # 从目录加载插件
                    dir_path = input(f"{Fore.CYAN}>> {Style.RESET_ALL}请输入插件目录路径 (默认: {config.plugin_dir}): ").strip()
                    if not dir_path:
                    ...
                    if not plugins:
                        ...
                        name = input(f"{Fore.CYAN}>> {Style.RESET_ALL}请输入要卸载的插件名称: ").strip()
                        if name:
                            pm.unregister(name)
